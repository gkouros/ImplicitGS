...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    for iteration in range(first_iter, opt.iterations+1):
        ...
        if args.use_gui:
            ...
            while network_gui.conn != None:
                try:
                    ...
                    if custom_cam != None:
                        net_image = render(custom_cam, gaussians, pipe, background, scaling_modifer)["render_net"]
        ...
        image, viewspace_point_tensor, visibility_filter, radii, image_net, radii_net = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"], render_pkg["render_net"], render_pkg["radii_net"]
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs)["render_net"], 0.0, 1.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[10_000, 20_000, 30_000, 40000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[10_000, 20_000, 30_000, 40000])
